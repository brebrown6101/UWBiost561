...
imp_numbers <- 1:15 #all implementations
trials <- 5 #five trials for each implementation
alpha_vec <- c(0.5, 0.95) #trying two different alpha levels
...
level_trial_list <- lapply(alpha_vec, function(alpha){
  ...
  trial_list <- lapply(1:trials, function(trial){
    ...
    # generate the data
    data <- UWBiost561::generate_partial_clique(n = 10, #graph of 10 nodes
                                                clique_fraction = 0.9,
    ...
    return(result_list) #listing the results in a list
  })
